...
```{r}
dados_amm <- "https://www.aos.wisc.edu/dvimont/MModes/RealTime/AMM.txt"

amm <- read.table(dados_amm, header = FALSE, na.strings = "-99.99")

colnames(amm) <- c("Year", "Mo", "SST", "Wind")
...
2.  
    ...
    a.  Leia diretamente no R os dados texto não estruturados disponíveis neste [link](https://raw.githubusercontent.com/lhmet/ADARdata/refs/heads/master/inst/extdata/previsoes-enos.txt) e salve-os em um arquivo texto nomeado `previsoes-enos.txt`, mantendo a formatação original dos dados.

```{r}
dados_enos <- "https://raw.githubusercontent.com/lhmet/ADARdata/refs/heads/master/inst/extdata/previsoes-enos.txt"

download.file(dados_enos,
              destfile = "previsoes-enos.txt",
              mode = "wb")
...
```{r}
txt <- readLines("previsoes-enos.txt")

lin_ini <- grep(pattern = "Dec 2020", x = txt)
lin_ini
...
# Texto da previsão
txt <- "
 -77 -96-104-111-127-142-137-999-999  D NASA GMAO
 -50 -76 -92-101-103-105 -93 -68-999  D NCEP CFSv2
 -36 -30 -25 -26 -27-999-999-999-999  D JMA
 -63 -52 -25  -4   7  16  27  41  58  D BCC_CSM11m
 -53 -52 -48 -39 -31 -21 -11   2  16  D SAUDI-KAU
   3  -2   8  28  41  50  59  67  60  D LDEO
 -49 -50 -52 -50 -49-999-999-999-999  D ECMWF
 -48 -60 -68 -74-999-999-999-999-999  D UKMO
 -42 -25   3  27  42  50  50  51  50  D KMA SNU
 -41 -50 -70 -87-101-107-107 -97 -82  D IOCAS ICM
 -61 -87-107-114-122-128-117 -90 -54  D COLA CCSM4
 -23 -24 -28 -36 -42-999-999-999-999  D MetFRANCE
 -35 -36 -38 -39 -39 -37 -32 -26 -21  D SINTEX-F
 -55 -55 -51 -45 -45 -43 -99 -99 -99  D CS-IRI-MM
 -89 -61 -16  21  41  52  63  80  99  D GFDL CM2.1
 -44 -50 -53 -56 -59 -61 -56 -44 -29  D CMC CANSIP
 -88 -79 -60 -49 -46 -40 -24   5  39  D GFDL FLOR
 -54 -71 -88 -96-100-109-113-102 -68  S NTU CODA
 -29 -26 -22 -16  -8   4  14  20  19  S CPC MRKOV
  27  20  16  16  21  17  17  21  37  S CPC CA
  14  31  48  65  65  65  65  51  38  S CSU CLIPR
 -36 -35 -38 -41 -45 -48 -50 -47 -39  S FSU REGR
 -26 -39 -52 -63 -69 -69 -64 -55 -44  S UCLA-TCD
 -37 -47 -60 -65 -67 -67 -60  -1  13  C CPC CONSOL
"
...
lista <- strsplit(linhas, "\\s+")


numeros <- t(sapply(lista, function(x) as.numeric(x[1:9])))

tipo_modelo <- sapply(lista, function(x) x[10])
modelo <- sapply(lista, function(x) paste(x[11:length(x)], collapse=" "))

prev_enos_jun2020 <- data.frame(
  numeros,
  tipo_modelo = tipo_modelo,
  modelo = modelo
)

colnames(prev_enos_jun2020)[1:9] <- c("JJA","JAS","ASO","SON","OND","NDJ","DJF","JFM","FMA")

str(prev_enos_jun2020)
tail(prev_enos_jun2020, 3)
...
write_xlsx(prev_enos_jun2020, "previsoes-enos.xlsx")
...
prev_enos_jun2020_import <- read_xlsx("previsoes-enos.xlsx")

str(prev_enos_jun2020_import)
```
...
```{r}
dados_aao <- "https://www.cpc.ncep.noaa.gov/products/precip/CWlink/daily_ao_index/aao/monthly.aao.index.b79.current.ascii.table"

aao <- read.table(dados_aao, header = TRUE, fill = TRUE)
...
```{r}
dados_xls <- "https://www.dropbox.com/s/4hc7x9s8pmfz7gq/_SS_ANNN_CITY_NAME_pressao_inst_max_min_vento_velo_dir_raja_radiacao_precipitacao_horario.xls?raw=1"

download.file(dados_xls,
              destfile = "dados_meteo.xls",
              mode = "wb")  
...
dados <- read_excel("dados_meteo.xls")
...
#### (i): Colunas = variáveis meteorológicas. Linhas = observações horárias.

#### (ii): Há 8 variáveis no arquivo

#### (iii): Provavelmente não, pois sensores falham
...
```{r}
# Tabela relacionando nomes originais e novos
nomes <- structure(list(
  nomes_originais = c(
    "data", "PRESSÃO ATMOSFERICA hPa", "VENTO VELOCIDADE",
    "VENTO DIREÇÃO graus", "RADIACAO GLOBAL KJM", "PRECIPITAÇÃO mm",
    "VENTO RAJADA MAXIMA ms", "PRESSÃO ATMOSFÉRICA MÁXIMA hPa",
    "PRESSÃO ATMOSFÉRICA MÍNIMA hPa"
  ),
  nomes_novos = c(
...
6.  
    ...
    a.  Utilizando o R, faça o *download* [deste arquivo netCDF](https://www.dropbox.com/scl/fi/xr3h829ujdu6y1i5rsyib/S_America_model_wtd_v2.nc?rlkey=gzs7yu2mt2wki6gq64ebizv2m&st=4le4xrre&dl=1) com dados de profundidade do lençol freático para a América do Sul.
...
nc_file <- nc_open("S_America_model_wtd_v2.nc")

names(nc_file$var)

var_name <- names(nc_file$var)

var_unidade <- sapply(nc_file$var, function(x) x$units)
...
# URL do arquivo
dados_rmm <- "http://www.bom.gov.au/climate/mjo/graphics/rmm.74toRealtime.txt"

res <- GET(dados_rmm, user_agent("Mozilla/5.0"))

txt <- content(res, "text", encoding = "UTF-8")

mjo_data <- read.table(text = txt, header = TRUE, sep = "", fill = TRUE, comment.char = "#")
...
```{r}
colnames(mjo_data) <- c("year", "month", "day", "RMM1", "RMM2", "phase", "amplitude", "status")

head(mjo_data$RMM1, 10)
```
...
# Salvar o data.frame mjo_data em arquivo .xls
WriteXLS("mjo_data", ExcelFileName = "mjo.xls")
...
mjo_import <- read_excel("mjo.xls")

class(mjo_import)

```

#### O arquivo foi importado como um tibble
...
n_linhas <- nrow(mjo_import)
n_colunas <- ncol(mjo_import)

cat("Número de linhas:", n_linhas, "\n")
cat("Número de colunas:", n_colunas, "\n")
...
linhas <- readLines(url_soi, warn = FALSE)

linhas <- linhas[linhas != ""]
linhas <- linhas[!grepl("^[A-Z]", linhas)]  # ignora linhas que começam com letras (como títulos)

dados_soi <- read.table(text = linhas, header = FALSE, fill = TRUE)

colnames(dados_soi) <- c("year", "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
...
9.  Importe no R o arquivo excel com a climatologia das temperaturas mínimas do INMET no período de 1981-2010, disponível [aqui](https://portal.inmet.gov.br/uploads/normais/Temperatura-Minima_NCB_1961-1990.xls). Mostre a estrutura dos dados e certifique-se de as colunas dos meses e ano são numéricas.
...
dados_inmet <- "https://portal.inmet.gov.br/uploads/normais/Temperatura-Minima_NCB_1961-1990.xls"

arquivo_local <- "Temperatura-Minima_NCB_1961-1990.xls"

df <- read_excel(arquivo_local)
names(df)

# Padronizar nomes (remover espaços e colocar primeira letra maiúscula)
